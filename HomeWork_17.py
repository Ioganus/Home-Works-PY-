...
# Соответствие условию ввода данных.
if " " not in sequence_numbers:
    print("\nВО ВВОДЕ ОТСУТСТВУЮТ ПРОБЕЛЫ (введите числа, отделяя каждое число пробелом.)")
    sequence_numbers = input('Введите целые числа через пробел: ')
if not is_int(sequence_numbers):
    print('\nВО ВВОДЕ СОДЕРЖАТСЯ НЕ СОДЕРЖАТЬСЯ ЦИФРЫ, ЛИБО ЦЕЛЫЕ ЧИСЛА (введите целые числа, согласно условиям ввода.)\n')
    print(error)
else:
