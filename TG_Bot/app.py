...
def valuess(message):
    bot.send_message(message.chat.id, 'Валюта доступная для конвертации:')
    for i in values:
...
def convert_result(message: telebot.types.Message):
    try:
        ...
        if len(val) != 3:
            raise APIException('Ошибка при вводе параметров для конвертации, воспользуётесь командой /help')

        base, quote, amount = val
        result = CryptoConverter.convert(base, quote, amount)
    except APIException as e:
        bot.reply_to(message, f'Неверные параметры ввода.\n {e}')
